...
| 9. Effets et modulateurs | Rack, catalogue, LFO/enveloppes faits ; les effets **une-frame** sont dessinés (`fs_effects.sc`, tenu conforme à `core/videofx` par `fx_check`) et pilotés par le mapping ; ceux qui lisent le clip à plusieurs positions (delay/traînées, slit scan) attendent les uploads multi-taps ; FFT non faite | `core/effect`, `core/videofx`, `core/modulator`, `ui/gpu_effects` |
